...
def _call_vcvarsall_and_return_env(arch):
  ...
  interesting = set(("INCLUDE", "LIB", "LIBPATH", "PATH", "SYSTEMROOT", "COMSPEC"))
  result = {}
...
def vcvarsall_end():
  global _old_environment
  os.environ.clear()
  os.environ.update(_old_environment)
  return
...
def get_file_last_modified_url(url):
  retvalue = ""
  curl_command = 'curl --connect-timeout 5 --max-time 10 --head %s' % (url)
