...
def fetch_repo(repo_url, branch_or_tag, target_dir=""):
  args = ["clone", "--depth", "1", "--branch", branch_or_tag, repo_url]
  if target_dir:
...
def make_common(build_func, cmake_args):
  ...
  if "windows" == base.host_platform():
    ...
    if config.check_option("platform", "win_arm64"):
      base.vcvarsall_start("x64_arm64")
...
def make_x265(base_dir, build_type):
  ...
  if not base.is_dir("x265_git"):
    fetch_repo("https://github.com/videolan/x265.git", "3.4", "x265_git")
