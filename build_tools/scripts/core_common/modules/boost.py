...
def make():
  ...
  if ("windows" == base.host_platform()):
    ...
    def find_cl_exe(vs_path):
      msvc_dir = os.path.abspath(vs_path + "/../../Tools/MSVC")
      cls = glob.glob(msvc_dir + "/*/bin/Hostx64/x64/cl.exe")
      if len(cls) > 0:
        return cls[0].replace("\\", "/")
      return ""

    cl_path = find_cl_exe(config.option("vs-path"))

    def has_boost_lib(plat):
      lib_dir = "../build/" + plat + "/lib"
      if not base.is_dir(lib_dir):
        return False
      return len(glob.glob(lib_dir + "/libboost_system*.lib")) > 0

    if (-1 != config.option("platform").find("win_64")) and not has_boost_lib("win_64"):
      boost_bat = []
      vcvarsall_call = ("call \"" + config.option("vs-path") + "/vcvarsall.bat\" x64")
      boost_bat.append(vcvarsall_call)
      if ("" != cl_path):
        boost_bat.append("echo using msvc : : \"" + cl_path + "\" ; > project-config.jam")
      else:
        boost_bat.append("call bootstrap.bat msvc")
      boost_bat.append("call b2.exe headers")
      boost_bat.append("call b2.exe --clean")
      boost_bat.append("call b2.exe --prefix=./../build/win_64 architecture=x86 link=static --with-filesystem --with-system --with-date_time --with-regex toolset=" + win_toolset + " address-model=64 install")
      base.run_as_bat(boost_bat)
    if (-1 != config.option("platform").find("win_32")) and not has_boost_lib("win_32"):
      boost_bat = []
      vcvarsall_call = ("call \"" + config.option("vs-path") + "/vcvarsall.bat\" x86")
      boost_bat.append(vcvarsall_call)
      if ("" != cl_path):
        boost_bat.append("echo using msvc : : \"" + cl_path + "\" ; > project-config.jam")
      else:
      ...
      boost_bat.append("call b2.exe headers")
      boost_bat.append("call b2.exe --clean")
      boost_bat.append("call b2.exe --prefix=./../build/win_32 architecture=x86 link=static --with-filesystem --with-system --with-date_time --with-regex toolset=" + win_toolset + " address-model=32 install")
      base.run_as_bat(boost_bat)
