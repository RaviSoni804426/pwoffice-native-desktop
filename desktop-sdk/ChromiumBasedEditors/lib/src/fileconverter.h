...
inline std::wstring GetPresentationThemes(CAscApplicationManager* pManager)
{
	std::wstring sThemesPath = pManager->m_oSettings.file_converter_path;
	sThemesPath += L"/../editors/sdkjs/slide/themes";
	return sThemesPath;
}
...
public:
	...
	virtual DWORD ThreadProc()
	{
		...
		if (m_oInfo.m_nCurrentFileFormat == AVS_OFFICESTUDIO_FILE_CROSSPLATFORM_PDF ||
			m_oInfo.m_nCurrentFileFormat == AVS_OFFICESTUDIO_FILE_CROSSPLATFORM_PDFA)
		{
			nOutputFormat = AVS_OFFICESTUDIO_FILE_DOCUMENT_DOCX;
		}
		else if (m_oInfo.m_nCurrentFileFormat & AVS_OFFICESTUDIO_FILE_DRAW)
		{
			nOutputFormat = 8197;
...
public:
	...
	virtual DWORD ThreadProc()
	{
		...
		oBuilder.WriteEncodeXmlString(sCMapFolder);
		oBuilder.WriteString(L"</m_sCmapDir>");
